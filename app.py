...
# 🔥 TERA CLOUDFLARE SUPER LINK 🔥
CF_HOST = "dry-wind-787d.krishnak962715.workers.dev"
        ...
        <body>
            <div class="video-container">
                <video id="player" playsinline controls crossorigin>
                    <source src="https://{CF_HOST}/stream/{message_id}" type="video/mp4" />
                </video>
            </div>
...
# ====================================================
# TELEGRAM BOT HANDLERS (AB DIRECT CLOUDFLARE LINK DEGA)
# ====================================================
...
async def media_cmd(client, message):
    try:
        ...
        # Yahan CF_HOST lag gaya hai!
        watch_link = f"https://{CF_HOST}/watch/{sent_msg.id}"
        stream_link = f"https://{CF_HOST}/stream/{sent_msg.id}"
        dl_link = f"https://{CF_HOST}/dl/{sent_msg.id}"
        
        reply_message = (
            f"🎬 **Link Generated Successfully!**\n\n"
            f"📥 **Download Link (Use this in Website):**\n`{dl_link}`\n\n"
            f"📺 **Web Player:**\n`{watch_link}`\n\n"
            f"⚡ **Cloudflare Super Stream:**\n`{stream_link}`"
        )
...
async def web_server():
    ...
    app.router.add_route("*", "/dl/{message_id}", stream_handler)
